...
export default function StockScreen() {
  ...
  const loadData = useCallback(async () => {
    const [prod, sales, sett] = await Promise.all([
      ProductionStore.getAll(),
      SaleStore.getAll(),
      SettingsStore.get(),
    ]);
    setTotalProd(prod.reduce((sum, p) => sum + p.quantity, 0));
    setTotalVendu(sales.reduce((sum, s) => sum + s.quantity, 0));
    setSettings(sett);
  }, []);
  ...
  const odooPackProduct =
    USE_ODOO_BACKEND && odooStock.data
      ? odooStock.data.products.find((product) => product.id === odooStock.data?.packProductId) ??
          ...
          <View className="flex-row justify-between mt-2">
            ...
            <Text
              className={`text-xl font-bold ${
                stockRestant > 50 ? "text-success" : stockRestant > 0 ? "text-warning" : "text-error"
              }`}
            >
              {stockRestant} packs
            </Text>
          ...
          <View className="mt-3 h-3 bg-border rounded-full overflow-hidden">
            <View
              className={`h-full rounded-full ${
                stockRestant > 50 ? "bg-success" : stockRestant > 0 ? "bg-warning" : "bg-error"
              }`}
              style={{
                width: `${Math.min((stockRestant / progressBase) * 100, 100)}%`,
              }}
            />
          </View>
        </View>
        ...
        {stockRestant <= 50 && (
          <View className="bg-warning/10 rounded-xl p-4 border border-warning/30">
            ...
            <Text className="text-xs text-muted mt-2">
              Il reste seulement {stockRestant} packs en stock. Pensez à lancer une
              nouvelle production.
            </Text>
          </View>
        )}
